...
$Debugcolor.DebugForegroundColor = "Blue"
	## 20200928.jmeyer.Removed Console color changes.
...
#$ErrorActionPreference = 'SilentlyContinue'
	## 20201002.jmeyer.Changed to Environmental variable so other functions can access it.
$env:FSpaceBefore = Get-CimInstance Win32_LogicalDisk -Filter "DeviceID='C:'" | Select-Object FreeSpace
...
{
	if ($Extras -contains "Offboarding")
	{
...
function UserCleanup ()
{
	...
	elseif ($Extras -contains "Offboarding")
	{
		Write-Host "Cleaning up unused User profiles in Users directory (Older than 30 days)..." -ForegroundColor Yellow
...
function DiskSpaceAfter ()
{
	...
	Format-Table -AutoSize | Out-String
	$FSpaceAfter = (Get-CimInstance Win32_LogicalDisk -Filter "DeviceID='C:'").FreeSpace / 1GB
	$Math = ($FSpaceAfter) - ($env:FSpaceBefore)
	## 20201002.jmeyer.Changed to Environmental variable so other functions can access it.
...
function WorkstationRestart ()
{
	...
	if ($Extras -notcontains "Offboarding")
	{
		...
		if ($Choice -eq $true)
		{
			...
			## Restarts computer
			## 20200911.jmeyer.Added option for restart using CimInstance. Currently commented out.
			#(Get-CimInstance -Class Win32_OperatingSystem).Win32Shutdown(2)
		...
		else
		{
			...
			## Logs out
			## 20200911.jmeyer.Using CimInstance for logout.
			(Get-CimInstance -ClassName Win32_OperatingSystem).Win32Shutdown(4)
		}
...
if ($Extras -contains "Kaseya")
{
	...
	{
		Write-Host "This version is designed to run on computers with less than 20% free space." -ForegroundColor Red
		Write-Host "We have more than 20% free space, exiting..." -ForegroundColor Red
		InstallCMTrace
		InstallRSATGPMT
		ScriptEnding
	}
